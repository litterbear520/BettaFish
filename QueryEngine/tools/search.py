"""
专为AI Agent设计的舆情搜索工具集 (Tavily)

此脚本将复杂的Tavily搜索功能分解为一系列目标明确、参数极少的独立工具
专为AI Agent调用而设计。Agent 只需根据任务意图选择合适的工具
无需理解复杂的参数组合。所有工具默认搜索“新闻”(topic='news')

主要工具:
- basic_search: 执行标准、快速的通用新闻搜索。
- deep_search: 对主题进行最全面的深度分析。
- search_last_day: 获取24小时内的最新动态。
- search_last_week: 获取过去一周的主要报道。
- search_images: 查找与新闻主题相关的图片。
- search_by_date: 在指定的历史日期范围内搜索。
"""
...
from retry import silent_retry, SEARCH_API_RETRY_CONFIG # type: ignore
...
class TavilyToolsClient:
    ...
    @silent_retry(SEARCH_API_RETRY_CONFIG, default_return=TavilyResponse(query="搜索失败"))
    ...
    def basic_search(self, query: str, max_results: int = 7) -> TavilyResponse:
        ...
        return self._tavily_search(
            ...
            search_depth="basic",
            include_answer=False # 关闭AI摘要快速搜索
        )

    def deep_search(self, query: str) -> TavilyResponse:
        """
    ...
    def search_last_day(self, query: str) -> TavilyResponse:
        """
        ...
        return self._tavily_search(query=query, time_range='d', max_results=10)

    def search_last_week(self, query: str) -> TavilyResponse:
        """
        ...
        return self._tavily_search(query=query, time_range='w', max_results=10)

    def search_images(self, query: str) -> TavilyResponse:
        """
    ...
    def search_by_date(self, query: str, start_date: str, end_date: str) -> TavilyResponse:
        """
...
if __name__ == "__main__":
    ...
    try:
        ...
        agency = TavilyToolsClient()

        # 场景1: Agent 进行一次常规、快速的搜索
        response1 = agency.basic_search(query="各大模型厂商最新进展", max_results=5)
        print_response_summary(response1)

        # 场景2: Agent 需要全面了解“全球芯片技术竞争”的背景
        response2 = agency.deep_search(query="Anthropic对X封禁")
        print_response_summary(response2)

        # 场景3: Agent 需要追踪“GTC大会”的最新消息
        response3 = agency.search_last_day(query="具身智能赛道风口")
        print_response_summary(response3)
        
        # 场景4: Agent 需要为一篇关于“自动驾驶”的周报查找素材
        response4 = agency.search_last_week(query="GitHub热门项目")
        print_response_summary(response4)
        
        # 场景5: Agent 需要查找“韦伯太空望远镜”的新闻图片
        response5 = agency.search_images(query="抖音萝卜纸巾猫")
        print_response_summary(response5)

        # 场景6: Agent 需要研究2025年第一季度关于“人工智能法规”的新闻
        response6 = agency.search_by_date(
            query="央行报告去年人均存款数量",
        ...
        print(f"初始化失败: {e}")
        print("请确保TAVILY_API_KEY环境变量已正确设置。")
    except Exception as e:
